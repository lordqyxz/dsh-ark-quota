...
- Host half (`lib/index.js`) signs the **control-plane OpenAPI** `GetCodingPlanUsage` (falling back to `GetAFPUsage` for Agent Plan) with your Volcengine **AK/SK** (SigV4 variant) behind a same-origin route (`/ark-quota`), because the OpenAPI gateway does not allow CORS from the DSH origin. No browser cookies or console session are sent upstream; local write routes use a per-instance CSRF capability.
...
- **Follows the selected model provider for quota viewing**: the card takes its identity from DSH's **llm provider route**. Switching the model in a session (composer seat or `/model` popup) makes the card follow to the bound Volcengine account's quota; switching sessions follows too. The dropdown offers "Auto-follow current model" (default) or pinning one provider; a pinned choice persists per route (`pinnedRoute`) and changes the quota view only, never the current session model.
- **Poker-like provider deck**: with multiple bound routes, the foreground quota card is layered over up to two labelled back cards, and a route change animates the new card into the front. If more routes exist, the deck shows the remaining count. An urgent-tier suggestion opens the provider with the most monthly headroom for viewing; it does not switch DSH's active model.
...
- Live maintenance: keys are read from the `ark-quota` settings namespace (`$DSH_HOME/settings.yaml`, hot-reloaded by `dsh-settings-file`). A change invalidates both the cache and in-flight refreshes immediately — **no server restart**.
...
3. Add an entry to your profile's `cordis.patch.yml`:
   ...
   - insert:
       - id: ark-quota
         ...
         config:
           accounts:
             - id: r_41571ef5438e3671     # internal credential-group id (stable route hash; normally auto-generated by the UI)
...
- With two or more bound routes a compact dropdown appears in the card header: the first item is "Auto-follow current model", the rest are routes. The card becomes a poker-like deck with one foreground card and up to two offset backs; a fourth or later route is summarized as "N more providers". Pinning writes `pinnedRoute`; choosing auto clears it. The urgent action is a quota-view shortcut only and never changes the current DSH model.
...
| key                | type     | default      | description                                     |
| ------------------ | -------- | ------------ | ----------------------------------------------- |
| `id`               | string   | auto         | Internal slug matching `/^[a-z][a-z0-9_]*$/`; also the snapshot-bucket key. Auto-generated on first route bind as `r_` plus a stable route hash; invalid/duplicate ids are dropped. |
...
```json
{
  ...
  "refreshMs": 300000,
  "hasReward": false,
  "accountId": "r_41571ef5438e3671",
  "accountLabel": "",
  "accounts": [{ "id": "r_41571ef5438e3671", "label": "", "configured": true, "providers": ["ark-coding-plan"] }],
  "routes": [{ "route": "ark-coding-plan", "name": "火山Coding Plan", "accountId": "r_41571ef5438e3671", "configured": true, "monthlyPct": 23 }],
  "activeAccountId": "r_41571ef5438e3671",
  "pinnedRoute": "",
  "quota": [
...
`GET /ark-quota/csrf` → a short-lived, per-plugin-instance CSRF capability for the browser client. State-changing POST requests must send it as `X-Ark-Quota-CSRF`; the route also accepts only loopback `Host` values.
