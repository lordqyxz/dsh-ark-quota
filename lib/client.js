window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// State-changing plugin routes require a per-server CSRF capability. Keep
		// it only in memory; a server restart naturally invalidates the old token.
		var csrfToken = null;
		...
		function getCsrfToken(force) {
			...
			csrfPromise = fetch("/ark-quota/csrf", { cache: "no-store" })
				.then(function (r) { return r.json(); })
				.then(function (json) {
					if (!json || json.ok !== true || typeof json.token !== "string" || json.token.length === 0) {
						throw new Error("无法取得 CSRF token");
					}
		...
		function postJson(path, body, retried) {
			return getCsrfToken(!!retried)
				.then(function (token) {
					return fetch(path, {
						method: "POST",
						headers: {
							"content-type": "application/json",
							"x-ark-quota-csrf": token
						},
						body: JSON.stringify(body)
					});
				})
				.then(function (response) {
					// A host restart rotates the token. Refresh it once, then surface
					// any real response to the caller.
		...
		function ensureStyles() {
			...
			style.textContent = [
				...
				"@keyframes dsh-ark-quota-card-in { from { opacity: 0; transform: translate3d(10px, 5px, 0) rotate(1deg); } to { opacity: 1; transform: translate3d(0, 0, 0) rotate(0deg); } }",
				...
				".arkq-deck { position: relative; width: 100%; }",
				".arkq-deck-front { position: relative; z-index: 3; animation: dsh-ark-quota-card-in .18s cubic-bezier(.2,.8,.2,1); }",
				".arkq-deck-back { position: absolute; inset: 0; pointer-events: none; transition: transform .18s cubic-bezier(.2,.8,.2,1); will-change: transform; }",
				".arkq-deck-more { position: absolute; right: 8px; bottom: 2px; z-index: 4; font-size: 10px; line-height: 14px; color: var(--dsw-alias-label-tertiary); pointer-events: none; }",
				"@media (prefers-reduced-motion: reduce) {",
				"  .arkq-spin, .arkq-skeleton, .arkq-deck-front { animation: none !important; }",
				"  .arkq-deck-back { transition: none !important; }",
				"}",
		...
		function quotaTargetKey(accountId, routeId) {
		...
		function useQuota(accountId, routeId) {
			var state = React.useState({ loading: true, data: null, error: null, targetKey: quotaTargetKey(accountId, routeId) });
			...
			var load = React.useCallback(function (force) {
				...
				var targetKey = quotaTargetKey(t.accountId, t.routeId);
				setState(function (prev) {
					var targetChanged = prev.targetKey !== targetKey;
					var initial = !prev.data && !prev.error;
					return {
						loading: !!(force || initial || targetChanged),
						data: targetChanged ? null : prev.data,
						error: force || initial || targetChanged ? null : prev.error,
				...
				fetch("/ark-quota" + (qs.length > 0 ? "?" + qs.join("&") : ""), opts)
					...
					.then(function (json) {
						...
						if (json && json.ok === true) {
							...
							setState({ loading: false, data: json, error: null, payload: json, targetKey: targetKey });
							...
							// 提供方查看器会消失（用户就没法查看另一个可用账号了）。
							setState({ loading: false, data: null, error: (json && json.message) || "查询失败", payload: json || null, targetKey: targetKey });
							if (timerRef.current) window.clearTimeout(timerRef.current);
					...
					.catch(function (e) {
						...
						if (seq !== seqRef.current) return;
						// 网络层失败拿不到 body：保留上一次的 payload，查看器不至于闪没。
						setState(function (prev) {
							return {
								loading: false,
								data: null,
			...
			var currentKey = quotaTargetKey(accountId, routeId);
			return {
				data: data.targetKey === currentKey ? data.data : null,
				loading: data.loading || data.targetKey !== currentKey,
				error: data.targetKey === currentKey ? data.error : null,
		...
		// 提供方查看器：头部右侧的紧凑小下拉，和刷新按钮待在同一个角落。
		//
		...
		function AccountSwitcher(_a) {
			...
			return jsxs("span", {
					title: selectValue === AUTO_OPTION
						? "自动查看：跟随当前会话的模型提供方（现在显示：" + currentLabel + "）。点击可固定查看某个提供方，不会更改当前会话模型。"
						: "已固定查看提供方：" + currentLabel + "。选「自动」可恢复随模型变化，不会更改当前会话模型。",
				style: {
		...
		function Card(_a) {
			...
			var viewRoute = _a.viewRoute, viewRouteLabel = _a.viewRouteLabel;
			...
			if (state.error) {
				return jsxs("div", {
					"data-arkq-route": viewRoute || undefined,
					...
					children: [
						jsxs("div", {
							...
							children: [
								...
								viewRouteLabel ? jsx("span", { style: { maxWidth: "120px", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", color: "var(--dsw-alias-label-tertiary)" }, title: viewRouteLabel, children: "· " + viewRouteLabel }) : null,
			...
			// 告急查看建议：短期档（5 小时/周）over 且 1 小时内会用完（或已
			// 撞线）时，找一个月度余量最足的已配置路由，头部给快速查看入口。
			// 这里故意只改变额度查看目标，不调用 DSH 的模型选择 API。
			var switchHint = null;
			(function () {
				...
				children: [
					jsxs("div", {
						...
						children: [
							...
							viewRouteLabel ? jsx("span", {
								style: { maxWidth: "130px", minWidth: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", fontSize: "10px", color: "var(--dsw-alias-label-tertiary)" },
								title: "当前查看：" + viewRouteLabel,
								children: "· " + viewRouteLabel
							}) : null,
							jsx("span", { style: { flex: "1", minWidth: 0 } }),
							// 头部右侧 = 操作区：告急查看建议 + 提供方查看器 + 刷新。
							switchHint ? jsx("button", {
								...
								title: (Object.prototype.hasOwnProperty.call(LEVEL_LABELS, switchHint.level)
									...
									+ "额度即将用完，查看余量更足的提供方「" + switchHint.label
									+ "」（近1月已用 " + fmtPct(clampPct(switchHint.monthlyPct)) + "%；不会更改当前会话模型）",
								...
								children: "查看「" + switchHint.label + "」额度"
							}) : null,
							jsx(AccountSwitcher, { routes: routes, currentRoute: currentRoute, onSelect: onSelectRoute, autoMode: autoMode }),
							jsx(RefreshButton, {
								onClick: onRefresh,
		...
		// 多个提供方时把额度查看做成一副小卡组：一张前景卡 + 最多两张
		// 背景卡。背景卡只提供层叠的视觉线索，不挂载 Card，也不会触发额外
		// 的额度轮询；原生下拉仍是唯一的键盘/鼠标切换入口。
		function QuotaDeck(_a) {
			var routes = Array.isArray(_a.routes) ? _a.routes : [];
			var frontRoute = _a.frontRoute || (routes[0] && routes[0].route) || null;
			var cardProps = _a.cardProps;
			var frontEntry = routeEntry(routes, frontRoute);
			var frontLabel = frontEntry ? (frontEntry.name || frontEntry.route) : frontRoute;
			var backRoutes = routes.filter(function (r) { return r.route !== frontRoute; }).slice(0, 2);
			var moreCount = Math.max(0, routes.length - 1 - backRoutes.length);
			var card = jsx(Card, Object.assign({}, cardProps, {
				viewRoute: frontRoute,
				viewRouteLabel: frontLabel
			}));
			if (routes.length < 2) return card;
			var backs = backRoutes.map(function (route, index) {
				var offset = (backRoutes.length - index) * 4;
				return jsx("div", {
					className: "arkq-deck-back",
					"aria-hidden": true,
					style: {
						zIndex: index + 1,
						transform: "translate(" + offset + "px, " + offset + "px) rotate(" + ((backRoutes.length - index) * 0.35) + "deg)",
						boxSizing: "border-box",
						borderRadius: "10px",
						border: "1px solid var(--dsw-alias-border-l2, rgba(128,128,128,0.2))",
						background: "var(--dsw-alias-bg-elevated, var(--dsw-alias-bg-base, transparent))",
						display: "flex", alignItems: "flex-start", justifyContent: "flex-end",
						padding: "6px 8px", overflow: "hidden"
					},
					children: jsx("span", {
						style: { maxWidth: "80%", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", fontSize: "10px", color: "var(--dsw-alias-label-tertiary)" },
						children: route.name || route.route
					})
				}, "back:" + route.route);
			});
			if (moreCount > 0) backs.push(jsx("span", {
				className: "arkq-deck-more",
				children: "另有 " + moreCount + " 个提供方"
			}, "more"));
			return jsx("div", {
				className: "arkq-deck",
				"data-arkq-deck-count": routes.length,
				style: { boxSizing: "border-box", paddingRight: backRoutes.length * 4 + "px", paddingBottom: backRoutes.length * 4 + "px" },
				children: backs.concat([jsx("div", {
					className: "arkq-deck-front",
					children: card
				}, "front:" + String(frontRoute || "default"))])
			});
		}
		...
		function ArkQuotaWidget(_a) {
			...
			var reload = q.load;
			var visibleSel = React.useState(null);
			var visibleRoute = visibleSel[0], setVisibleRoute = visibleSel[1];
			React.useEffect(function () {
			...
			// q.payload 可以保留旧目标的路由清单用于切换器，但 q.data 只在
			// 响应目标与当前目标一致时才有值，避免新路由标签配上旧额度。
			var data = q.data;
			var payload = q.payload || data || null;
			...
			var responseRoute = queryRoute || (function () {
				if (!data) return null;
				var id = data.accountId || data.activeAccountId;
				var acc = accounts.find(function (x) { return x.id === id; });
				return acc && Array.isArray(acc.providers) && acc.providers.length > 0 ? acc.providers[0] : null;
			})();
			// 只有新目标的额度响应到达后，前景卡才换牌；切换请求等待期间
			// 保留旧前景，卡面进入动画因此不会把旧额度和新标签混在一起。
			React.useEffect(function () {
				if (!data || !responseRoute || responseRoute === visibleRoute) return;
				setVisibleRoute(responseRoute);
			}, [data, responseRoute, visibleRoute]);
			var frontRoute = visibleRoute || displayRoute || responseRoute;
			var onSelectRoute = React.useCallback(function (route) {
				if (route === null) {
					// 选回「自动跟随」：清本地固定 + 通知宿主解除 pin。
					setPickedRoute(null);
					postJson("/ark-quota/routes", { action: "pin", route: "" }).catch(function () {});
					return;
				}
				setPickedRoute(route);
				// 写回宿主固定到该路由；失败也不影响本次查看（本地态已切）。
				postJson("/ark-quota/routes", { action: "pin", route: route }).catch(function () {});
			}, []);
			var cardProps = {
				state: { data: data, error: q.error },
				loading: q.loading,
				routes: routes,
			...
			if (!wide) return jsx(RailPill, { state: { data: data, error: q.error } });
			return jsx(QuotaDeck, { routes: routes, frontRoute: frontRoute, cardProps: cardProps });
		}
		...
		function ArkQuotaSettingsCard() {
			...
			var currentBound = currentRoute !== null;
			...
			var onRefreshChange = function (e) {
				...
				postJson("/ark-quota/settings", { refreshMs: ms })
					.then(function (r) { return r.json(); })
					.then(function (json) {
						if (json && json.ok === true) {
			...
			var onSave = function () {
				...
				postJson("/ark-quota/credentials", {
			...
			var onClearRoute = function () {
				...
				postJson("/ark-quota/routes", { action: "clear", route: editingRoute })
					.then(function (r) { return r.json(); })
					.then(function (json) {
						if (json && json.ok === true) {
