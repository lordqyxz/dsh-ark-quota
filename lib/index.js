...
// open.volcengineapi.com. No browser cookies or console session are sent
// upstream; local state-changing routes use a per-instance CSRF capability.
//
...
/**
 * 把 llm 提供方路由 id 映射成合法的内部账号 id（落盘分桶键）。
 *
 ...
 * 同源，且要成为快照分桶键）。使用路由 id 的稳定摘要，避免
 * `ark-prod` / `ark_prod` 这类不同路由清洗后落到同一个账号 id。
 * 前缀补 `r_`，把路由衍生 id 放进独立空间。
 */
export function accountIdFromRoute(providerId) {
  const route = str(providerId);
 ...
 * 显示名当标签。即使摘要 id 恰好已被历史数据占用，也会生成带稳定序号的
 * 新 id，绝不把一个新路由静默挂到别的账号上。
...
export function ensureRouteAccount(accounts, routeId) {
  ...
  const baseId = accountIdFromRoute(routeId);
 ...
 *
 * 这不是单靠 Origin/Host 相等来判断：DNS rebinding 可以让攻击者控制
 * Origin 的主机名，所以写请求还必须携带仅能由同源页面读取的随机 token。
 * 同时把 Host 限制在 DSH 支持的 loopback 名称，避免攻击者用自有域名取得
 * token。Sec-Fetch-Site / Origin 只是额外的跨站预过滤。
...
/** Only DSH's loopback listener may use the state-changing route surface. */
export function isLoopbackHost(req) {
  const host = req?.headers?.host;
  // Node's normal HTTP/1.1 requests always have Host. Keep test/non-HTTP
  // adapters compatible; the CSRF token check still protects their POSTs.
...
function hasValidCsrf(req, expected) {
  const supplied = req?.headers?.["x-ark-quota-csrf"];
...
/** POST route guard: loopback Host + same-origin metadata + CSRF capability. */
function rejectCrossOrigin(req, res, csrfToken) {
  if (!isLoopbackHost(req)) {
    sendJson(res, 403, {
      ok: false,
      code: "invalid-host",
      message: "拒绝非本机 Host 请求"
    });
    return true;
  }
...
export function apply(ctx, config) {
  ...
  // Per plugin instance; never persist or log it. Browser code obtains it from
  // the same-origin endpoint before any state-changing POST.
  const csrfToken = randomBytes(32).toString("base64url");
  // 每个账号一份缓存：不同账号打的是不同上游账号，绝不能共用一条缓存。
  const caches = new Map(); // accountId -> { at, payload, revision, fingerprint }
  // 每个账号一个在途请求：缓存过期瞬间的并发请求（多窗口、保存后 fan-out
  // 刷新）只允许打一次上游，其余等同一个 Promise。
  const inflight = new Map(); // accountId -> { revision, fingerprint, promise }
  ...
  ctx.effect(() => {
    ...
    const unwatch = scope.watch(() => {
      configRevision += 1;
      caches.clear();
      // Clearing the map prevents a request started with old credentials from
      // being reused by new callers. Its finally handler is identity-guarded
      // below, so it cannot delete a newer entry for the same account.
      inflight.clear();
  ...
  // Never log this value. It only lets an in-flight response prove that the
  // credentials/configuration it used are still the current ones.
  ...
  const isCurrentRefresh = (account, revision, fingerprint) => {
    if (revision !== configRevision) return false;
    const current = accountOf(account.id);
    return current !== null && accountFingerprint(current) === fingerprint;
  };
  ...
  const cacheFresh = (accountId) => {
    ...
    return entry !== undefined && account !== null &&
      entry.revision === configRevision &&
      entry.fingerprint === accountFingerprint(account) &&
      Date.now() - entry.at < effective().refreshMs;
  };
   ...
   */
  const refresh = async (account, revision = configRevision, fingerprint = accountFingerprint(account)) => {
    const assertCurrent = () => {
      if (!isCurrentRefresh(account, revision, fingerprint)) {
        throw upstreamError("stale-config", "ark-quota: 丢弃配置变更前发起的额度响应");
      }
    };
    assertCurrent();
    const cfg = effective();
    ...
    assertCurrent();
    caches.set(account.id, { at: nowMs, payload, revision, fingerprint });
  ...
  const refreshShared = (account) => {
    ...
    if (existing && existing.revision === revision && existing.fingerprint === fingerprint) {
      return existing.promise;
    }
    const job = Promise.resolve()
      .then(() => refresh(account, revision, fingerprint));
    const entry = { revision, fingerprint, promise: job };
    inflight.set(account.id, entry);
    // Do not let an old job's cleanup remove a newer job installed after a
    // settings change. Attach a rejection handler to the cleanup branch so the
    // original promise remains the one callers observe without an unhandled
    // rejection from Promise.finally/then bookkeeping.
    job.then(
      () => { if (inflight.get(account.id)?.promise === job) inflight.delete(account.id); },
      () => { if (inflight.get(account.id)?.promise === job) inflight.delete(account.id); }
    );
    return job;
  };
   ...
   *
   * 查看建议用：当前账号短期额度告急时，前端要能直接看到哪个账号月度余量
  ...
  const handler = async (req, res) => {
    ...
    try {
      ...
      let payload;
      if (fresh) {
        payload = caches.get(account.id).payload;
      } else {
        try {
          payload = await refreshShared(account);
        } catch (error) {
          // A settings update may have invalidated this request while it was
          // in flight. Re-resolve the requested target and retry once, so the
          // caller gets current credentials or an honest current-target error.
          if (error?.code !== "stale-config") throw error;
          const retry = routeParam
            ? accountForRouteId(routeParam)
            : accountOf(params.get("account"));
  ...
  // Same-origin browser code fetches this capability immediately before a
  // mutating request. It is intentionally scoped to this plugin instance and
  // is never included in status/quota payloads or logs.
  ctx.effect(() => ctx.webServer.register({
    kind: "exact",
    path: "/ark-quota/csrf",
    handler: async (req, res) => {
      ...
      if (!isLoopbackHost(req)) {
        sendJson(res, 403, { ok: false, code: "invalid-host", message: "拒绝非本机 Host 请求" });
        return;
      }
      res.writeHead(200, {
        "content-type": "application/json; charset=utf-8",
      ...
      res.end(req.method === "HEAD" ? "" : JSON.stringify({ ok: true, token: csrfToken }));
    }
  }), "ark-quota: /ark-quota/csrf route");
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      if (rejectCrossOrigin(req, res, csrfToken)) return;
